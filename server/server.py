...
@app.route('/morelab/api/v1.0/municipality/<string:municipality>/<int:page>')
def municipality_pagination(municipality, page):
    offset = (page - 1) * 20
    cur.execute("SELECT * FROM tweets WHERE municipality = '%s' ORDER BY datetime DESC LIMIT 20 OFFSET %s;" % (municipality, offset))
    items = cur.fetchall()
    json_str = '['
    i = 0
    for item in items:
...
@app.route('/morelab/api/v1.0/stream/<int:page>')
def pagination(page):
    offset = (page - 1) * 20
    cur.execute("SELECT * FROM tweets ORDER BY datetime DESC LIMIT 20 OFFSET %s;" % offset)
    items = cur.fetchall()
    json_str = '['
    i = 0
    for item in items:
